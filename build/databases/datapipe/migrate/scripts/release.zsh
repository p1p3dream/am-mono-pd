...
SCRIPTS_DIR=$(yq '.scripts_dir.'${ABODEMINE_NAMESPACE} config.yaml)

ssh ${ABODEMINE_BASTION_SSH} -- mkdir -p \
    ${CONFIG_DIR} \
    ${MIGRATIONS_DIR} \
    ${SCRIPTS_DIR}
...
rsync -aP --delete ${ABODEMINE_WORKSPACE}/code/sql/databases/${ABODEMINE_DATABASE_NAME}/scripts/ ${ABODEMINE_BASTION_SSH}:${SCRIPTS_DIR}/
